...
zstyle ':fzf-tab:complete:git-checkout:*' fzf-preview \
	'git log --color=always --oneline --graph --date=short $word'
...
function copyTerm() {
  ...
  if [[ -n "${aliases[$target]}" ]]; then
    local alias_value="${aliases[$target]}"
    target="${alias_value#ssh }"
    echo "Resolved alias '$1' to '$target'"
  fi
  ...
  echo "Copying $TERM terminfo to $target..."
  infocmp "$TERM" | ssh $target "tic -x -o ~/.terminfo /dev/stdin"
}
...
alias spot-prod="ssh debian@app.spotpilot.ch"
...
alias intr='z_launch INTR intr'
alias args='z_launch ARGS args'
